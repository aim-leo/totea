...
class CategoryService extends ToteaService {
  ...
  _loopGetSubCate(
    ...
    for (const item of nodeList) {
      ...
      const childCates = list.filter(s => s.parent && s.parent.id === item.id)

      if (childCates && childCates.length > 0) item.children = this._loopGetSubCate(childCates, list)
      
      if (!item.children || item.children.length === 0) item.children = undefined
